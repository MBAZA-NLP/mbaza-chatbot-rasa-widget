...
console.log("Mbaza chat...");

const imagesPath = {
  open: "https://dev.mbaza.digital/static/images/widget_open.svg",
  close: "https://dev.mbaza.digital/static/images/widget_close.svg",
};
...
export default class Chatroom extends Component<ChatroomProps, ChatroomState> {
  ...
  render() {
    ...
    return (
      <div>
        <div
          className={classnames("chat-container", isOpen ? "open" : "closed")}
        >
          <div className={classnames("chatroom", isOpen ? "open" : "closed")}>
            <div className="header">
              {" "}
              {/*onClick={this.props.onToggleChat}*/}
              <img src="https://dev.mbaza.digital/static/images/bot.svg" />
              <div className="hh3">{this.props.title}</div>
            </div>
            ...
            <form className="input" onSubmit={this.handleSubmitMessage}>
              ...
              <input
                type="image"
                name="submit"
                src="https://dev.mbaza.digital/static/images/submit.svg"
                height="20px"
                width="21px"
              />
              {this.props.speechRecognition != null ? (
                <SpeechInput
                  ...
                  onSpeechInput={(message) =>
                    this.handleInputChange(message, true)
                  }
        ...
        <div className="launcher-container">
          <button
            ...
            className={classnames("launcher", isOpen ? "opened" : "closed")}
          >
            <img
              src={imagesPath[imageName]}
              width={`${imageName == "close" ? "15px" : "30px"}`}
            />
          </button>
        </div>
      </div>
